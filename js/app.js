(() => {
    ...
    (() => {
        ...
        document.addEventListener("DOMContentLoaded", (() => {
            const pageContainer = document.querySelector("main[data-page-id]");
            if (!pageContainer) return;
            const pageId = pageContainer.getAttribute("data-page-id");
            if (!pageId) return;
            console.log("Current page ID:", pageId);
            const buttons = document.querySelectorAll(`.aside-reg__buttons[data-page-id="${pageId}"] button`);
            const textBlocks = document.querySelectorAll(`.registration__content[data-page-id="${pageId}"]`);
            const dynamicImage = document.getElementById("dynamic-image");
            if (!buttons.length || !textBlocks.length || !dynamicImage) return;
            const backButton = document.getElementById("back-button");
            if (backButton) backButton.addEventListener("click", (() => {
                if (window.history.length > 1) window.history.back(); else alert("There is no previous page to return to.");
            }));
            const imagesConfig = {
                page1: {
                    1: "img/registration/banner1.webp",
                    2: "img/registration/banner2.webp",
                    3: "img/registration/banner3.webp"
                },
                page2: {
                    1: "img/resources/banner1.webp",
                    2: "img/resources/banner2.webp",
                    3: "img/resources/banner3.webp"
                }
            };
            buttons.forEach((button => {
                button.addEventListener("click", (() => {
                    const blockNumber = button.getAttribute("data-hide-block");
                    buttons.forEach((btn => btn.classList.remove("active")));
                    button.classList.add("active");
                    textBlocks.forEach((block => block.classList.remove("active")));
                    const targetBlock = document.querySelector(`.registration__content[data-page-id="${pageId}"][data-block-number="${blockNumber}"]`);
                    if (targetBlock) targetBlock.classList.add("active"); else console.warn(`Block with number ${blockNumber} was not found!`);
                    const images = imagesConfig[pageId];
                    if (images && images[blockNumber]) {
                        dynamicImage.src = images[blockNumber];
                        dynamicImage.alt = `Block image ${blockNumber} for the page ${pageId}`;
                    } else console.warn(`No image was found for block ${blockNumber} on page ${pageId}!`);
                }));
            }));
        }));
        document.addEventListener("DOMContentLoaded", (function() {
            if (!window.location.pathname.includes("club-shop.html")) return;
            const productList = document.querySelector(".products__list");
            const products = Array.from(productList.querySelectorAll(".item-card"));
            const select = document.getElementById("price-sort");
            const productsCountElement = document.querySelector(".products__count");
            if (!productsCountElement) {
                console.warn("The .products__count element was not found");
                return;
            }
            function updateProductsCount() {
                const currentCount = productList.querySelectorAll(".item-card").length;
                productsCountElement.textContent = `Total items ${currentCount}`;
            }
            updateProductsCount();
            select.addEventListener("change", (function() {
                ...
                if (selectedOption === "1") {
                    const sortedProducts = products.slice().sort(((a, b) => {
                        const priceA = parseFloat(a.querySelector(".item-card__price").textContent.replace("$", "").trim());
                        const priceB = parseFloat(b.querySelector(".item-card__price").textContent.replace("$", "").trim());
                        return priceA - priceB;
                    }));
                    productList.innerHTML = "";
                    sortedProducts.forEach((item => productList.appendChild(item)));
                    updateProductsCount();
                }
                if (selectedOption === "2") {
                    const sortedProducts = products.slice().sort(((a, b) => {
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            if (!document.getElementById("highlights-page")) return;
            const cards = document.querySelectorAll("#highlightsList .highlights__slide");
            const cardsPerLoad = 9;
            let currentCount = 0;
            const loadMoreBtn = document.querySelector("#loadMoreBtn");
            function showCards() {
                const maxCount = currentCount + cardsPerLoad;
                for (let i = currentCount; i < maxCount && i < cards.length; i++) {
                    const card = cards[i];
                ...
                if (currentCount >= cards.length) loadMoreBtn.style.display = "none";
            }
            cards.forEach((card => {
        ...
        document.addEventListener("DOMContentLoaded", (() => {
            const pageContainer = document.getElementById("single-news");
            if (!pageContainer) return;
            const backButton = document.getElementById("back-button");
            if (backButton) backButton.addEventListener("click", (() => {
        ...
        function initSliders() {
            if (document.querySelector(".swiper-main")) new swiper_core_Swiper(".swiper-main", {
            ...
            if (document.querySelector(".swiper-highlights")) new swiper_core_Swiper(".swiper-highlights", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                speed: 800,
            ...
            if (document.querySelector(".swiper-shop-kit")) new swiper_core_Swiper(".swiper-shop-kit", {
                ...
                lazy: true,
                navigation: {
                    prevEl: ".shop-kit-swiper-button-prev",
                    nextEl: ".shop-kit-swiper-button-next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1,
